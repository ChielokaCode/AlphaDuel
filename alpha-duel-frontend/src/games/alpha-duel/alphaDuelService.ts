import { Client as AlphaDuelClient, type Game } from './bindings';
...
/**
 * Service for interacting with the AlphaDuel game contract
 */
export class AlphaDuelService {
  private baseClient: AlphaDuelClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new AlphaDuelClient({
  ...
  private createSigningClient(
    ...
    return new AlphaDuelClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new AlphaDuelClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
  ...
  /**
   * Make a guess in the alpha-duel game
   */
  async makeGuess(
    sessionId: number,
    playerAddress: string,
    guess: number[], // AlphaDuel expects an array of numbers
    signer: Pick<contract.ClientOptions, 'signTransaction' | 'signAuthEntry'>,
    authTtlMinutes?: number
  ) {
    if (!Array.isArray(guess) || guess.length !== 3) {
      throw new Error('Guess must be an array of 3 numbers');
    }
  ...
  async commitGuess(
    ...
    const client = this.createSigningClient(playerAddress, signer);
    const tx = await client.commit_guess({
      session_id: sessionId,
      player: playerAddress,
      guess_commitment: guessCommitmentBuffer,
    }, DEFAULT_METHOD_OPTIONS);
...
async revealWinnerWithProof(
  ...
  try {
    ...
    try {
      ...
      // Extract public outputs as number
      const publicOutputs = returnValue.toString(); // e.g., "0x01"
      winnerFlagNum =
        typeof publicOutputs === 'string' && publicOutputs.startsWith('0x')
          ? parseInt(publicOutputs, 16)
          : Number(publicOutputs);

      console.log("Generated proof with length:", proof.proof);
      console.log("Public outputs:", publicOutputs);
      console.log("Extracted winner flag:", winnerFlagNum);

      const isValid = await backend.verifyProof(proof);
      console.log("Proof verification result:", isValid);

    } catch (err) {
      console.error('❌ Error during Noir execution or proof generation:', err);
      throw new Error(`Noir execution/proof failed: ${err instanceof Error ? err.message : String(err)}`);
    }
