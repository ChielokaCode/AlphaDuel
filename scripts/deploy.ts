...
async function ensureTestnetFunded(address: string): Promise<void> {
  if (await testnetAccountExists(address)) {
    console.log(`✅ Account ${address} exists`);
    return;
  }
  console.log(`⚠️ Account ${address} does not exist. You need to fund it manually on testnet.`);
  throw new Error(`Admin account ${address} not funded`);
}
...
const adminAddressS = adminKeypair.publicKey();

console.log("Funding admin:", adminAddressS);
console.log("Admin secret:", adminKeypair.secret());

walletAddresses.admin = adminAddressS;
console.log('✅ Admin address:', walletAddresses.admin);
await fetch(`https://friendbot.stellar.org?addr=${walletAddresses.admin}`);
